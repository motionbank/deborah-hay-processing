...
PieceMakerApi api;
Group piece;
org.piecemaker2.models.Event[] videos;
...
void setup ()
{
    ...
    api = new PieceMakerApi( this, "http://localhost:9292", //"http://deborah-hay-pm2.herokuapp.com",
                "0310X9kgAiTWuMIt" );
    //api.whoAmI( api.createCallback( "meMyselfAndI" ) );
    api.listAllGroups( api.createCallback( "piecesLoaded" ) );
}

void meMyselfAndI ( User u )
{
    println(String.format("%d %s %s",u.id,u.email,u.user_role_id));
}
