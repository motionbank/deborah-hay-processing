 ...
 *
 *    Repairing data by interpolating 0,0 points from last to next known point
 *
 *    P2.0
 *    fjenett 20130212
 */
 ...
 String repairedExt = "_interpolated.txt";
 ...
 void findFiles ()
 {
     ...
     for ( String folder : rootDir.list() )
     {
         ...
         {
             if ( !file.equals("Tracked3DPosition.txt") ) continue;
 ...
 void convertFiles ()
 {
     ...
     for ( String file : files )
     {
         ...
         float[][] lessLines = new float[lines.length][dimension];
         
         for ( int i = 0; i < lessLines.length; i++ )
         {
             String[] point3D = split(lines[i], " ");
         ...
         saveStrings( file.replace( ".txt", repairedExt ), lessLinesStr );
