   ...
   float segHeight = 20;
    ...
    for (int i=0; i<textSegments.length(); i++) {
      ...
      //if ( i >= skipStartMarker ) {
        ...
        // traveled total
        x = sw;
        w = maxL* (vSeg.positions.total / videoData.positions.total);
        fill(0,255,0);
        ...
        // traveled vid diff
        x = sw*3;
        w = maxL*( (vSeg.positions.total / videoData.positions.total) - vSeg.relLength() );
        if (w>0) fill(0,255,0);
        else fill(0,0,255);
        rect(x,y+1, w,h);
        ...
        // traveled text diff
        x = sw*4;
        w = maxL*( (vSeg.positions.total / videoData.positions.total) - tSeg.relLength() );
        if (w>0) fill(0,255,0);
        else fill(255,0,0);
        rect(x,y+1, w,h);
